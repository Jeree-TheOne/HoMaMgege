...
private:
    ...
    void CreateEquipment(string name, int damage, int hp, int armor, int intellect, int magicstrength) {
        int i;
        int y;//переменная которая будет потом когда я напишу рандом коннектиться с рандомом
        ...
        {
        cout << "Вам выпал: ";
        if (ekipirovka == 1) //ар это оружие которое потом будет коннектится с рандомом я еще ничего не сделала, сделаю когда дадут пизды, сейчас переименую, каждому предмету присваивается своё число
